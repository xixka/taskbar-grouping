...
- `cargo build --release` —— 唯一经验证的构建命令；提取自
  ...
  门禁 = 该命令在 CI 绿灯。
- `ci/runtime-smoke.ps1` —— CI 运行时冒烟（任务 9 + 13 + 14 + 19，
  ...
  无参启动 → 菜单启停 watch/还原/退出，全程无 Ctrl+C）与任务 19 自启
  （Phase I：双线路 install 注册值逐字节比对、status 三块状态、幂等卸载、
  用法错误退出码 2）；47 项断言；仅由 CI 执行，本地未验证。
